...
Открыть **http://127.0.0.1:8765**. Исходный февральский архив и обученная модель уже сохранены; для просмотра и локального расчёта интернет не нужен. Сервер слушает только этот компьютер. `Ctrl+C` останавливает сервер; другой порт задаётся `--port 8766`.

Для режима **«Агент OpenAI»** создать локальный `.env` по примеру `.env.example`:
...
Ключ хранится только на сервере, исключён из Git и не передаётся браузеру. Значения системного окружения имеют приоритет перед `.env`. Просмотр страницы не вызывает платных API; запрос выполняется после нажатия «Рассчитать прогноз» в режиме OpenAI. По умолчанию используется локальный расчёт, явно отмеченный как режим без LLM. При ошибке OpenAI приложение показывает ошибку, а локальный режим остаётся доступен.

Выбрать выпуск, режим и турбину → рассчитать → увидеть реальные вызовы инструментов → скачать CSV.

Рабочий экран позволяет сравнивать обе турбины, переключать горизонт 24/48 часов
и показатель «мощность/ветер». Ползунок под графиком и наведение выбирают час:
...
Численный режим использует те же проверяемые инструменты в фиксированном порядке. Он позволяет повторить прогноз без OpenAI; живой LLM-вызов проверяется отдельно.
