...
**От погодного прогноза до почасовой мощности двух ветровых турбин.**

[![Смотреть 24-секундное демо WindOps](brag-output/brag.jpg)](brag-output/brag.mp4)

[Смотреть демо · MP4](brag-output/brag.mp4) · [Запустить локально](#запустить-приложение) · [Готовые прогнозы](#готовые-результаты-февраля) · [Результаты моделей](outputs/gfs_ml/RESULTS.md)

WindOps объединяет прогноз на **48 часов**, сравнение выпусков, проверку по факту и вопросы AI-агенту. Приложение воспроизводит архив февраля 2026: агент OpenAI получает погоду NOAA, проверяет её доступность на момент выпуска, вызывает сохранённую модель и объясняет результат. **Мощность рассчитывает численная модель; OpenAI управляет инструментами и даёт объяснение.**
...
- [Видео](brag-output/brag.mp4), [обложка](brag-output/brag.jpg), [текст для публикации](brag-output/share-copy.txt).
