...
Ключ хранится только на сервере, исключён из Git и не передаётся браузеру. Значения системного окружения имеют приоритет перед `.env`. Просмотр страницы не вызывает платных API; запрос выполняется после нажатия единственной кнопки «Рассчитать прогноз». При ошибке OpenAI приложение показывает причину; после её устранения можно повторить расчёт.
