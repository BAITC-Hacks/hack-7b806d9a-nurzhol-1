...
The home route introduces WindOps with a separate landing page; the full workspace lives at `/app`. The landing keeps the same palette and local fonts at a larger reading scale. Its interactive two-turbine preview reads the actual January 31 issue through the existing forecast API, offers power/wind and hourly inspection, and is explicitly labelled as an archive example. Product claims describe shipped capabilities; no generated forecasts, live-data claims or invented accuracy metrics. Header, hero and final actions open the workspace, whose brand links back home.
